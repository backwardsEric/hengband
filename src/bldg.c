		...
		case BACT_REST: /* Rest for the night */
			...
			else
			{
				...
				if ((prev_hour >= 6) && (prev_hour <= 17)) 
					do_cmd_write_nikki(NIKKI_BUNSHOU, 0, _("宿屋に泊まった。", "stayed during the day at the inn."));
				else
					do_cmd_write_nikki(NIKKI_BUNSHOU, 0, _("宿屋に泊まった。", "stayed overnight at the inn."));
				...
				if (ironman_nightmare)
				{
					...
					do_cmd_write_nikki(NIKKI_BUNSHOU, 0, _("悪夢にうなされてよく眠れなかった。", "had a nightmare."));
				}
				else
				{
					set_blind(0);
					...
					if ((prev_hour >= 6) && (prev_hour <= 17))
					{
						...
						do_cmd_write_nikki(NIKKI_BUNSHOU, 0, _("夕方を迎えた。", "awoke refreshed."));
					}
					else
					{
						msg_print(_("あなたはリフレッシュして目覚め、新たな日を迎えた。", "You awake refreshed for the new day."));
						do_cmd_write_nikki(NIKKI_BUNSHOU, 0, _("すがすがしい朝を迎えた。", "awoke refreshed."));
					}
				}
			}
			break;
